...
export const init = (event) => {
  ...
  (($) => {
     ...
     */
    const [THEME_CAMELCASED]Settings = {};

    if (typeof window?.[THEME_CAMELCASED]Settings?.colors === 'object') {
      [THEME_CAMELCASED]Settings.colors = window.[THEME_CAMELCASED]Settings.colors;
    }

    DEBUG &&
      console.log({
        msg: '[THEME_CAMELCASED]Settings are loaded.',
        [THEME_CAMELCASED]Settings: [THEME_CAMELCASED]Settings,
      });
